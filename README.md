...
4) 이미지는 카메라 정면 약 1m 앞(기본 너비 약 0.75m)으로 바로 나타나며, 한 손가락 드래그로 이동, 두 손가락 핀치로 크기 조정(0.3~2.5m)할 수 있습니다. 이미지와 함께 따라오는 “사이트로 이동” 버튼을 눌러 `http://www.infocus1.co.kr`(80 포트) 로 접속할 수 있으며, 홀로그램 느낌의 글로우가 표시됩니다.

## 특징
- Three.js + WebXR로 카메라 정면에 이미지 배치, 드래그/핀치로 이동·스케일, 이미지 옆 “사이트로 이동” 버튼으로 사이트 연결
