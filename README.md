...
3) 페이지를 열면 곧바로 카메라 권한을 요청하고 AR 세션을 띄웁니다. 자동 차단되면 화면 한 번 탭하면 시작됩니다.  
4) 이미지는 카메라 정면 약 1m 앞에 바로 나타나며, 한 손가락 드래그로 이동, 두 손가락 핀치로 크기 조정(0.3~2.5m 범위)할 수 있습니다.

## 특징
- Three.js + WebXR로 카메라 정면에 이미지 배치, 드래그/핀치로 이동·스케일
